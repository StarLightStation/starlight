...
public class LoginController {
	 ...
	 * 요청 페이지 : login.tag(공통), subscription.tag(subscription.jsp) 
	 * 리턴 값 : model : X / 세션 : X 
	 * 기능 : login.jsp 로 이동
	 */
	...
	public String login(MemberVO mVO, HttpSession session, Model model) {
		...
		if (mVO != null) { // 회원정보가 있다면,
			...
			return "sweetAlert.do";    //   sweetAlert 창을 띄우기 위한 페이지로 이동 하기.
		}
	...
	/*
	 * 요청 값 : VO에 존재 : X / 세션 : X / VO에 없음 : googleEmail, googleName 
	 * 요청 페이지 : X 
	 * 리턴 값 : model : X / 세션 : mID, mName
	 * 기능 : 구글 회원가입, 구글 로그인
	 */
	...
	public String signup(MemberVO mVO, UsecouponVO ucVO, Model model) {
		...
		//  View 에서 보내준 추천인이 유효한 추천인 이였다면,
			...
			usecouponService.insert(ucVO);  //  신규 회원 에게 추천인 쿠폰을 지급 하기.

			ucVO.setmID(mVO.getTmpmID());   //  기존 회원의 mID 값을 set 하기.
			usecouponService.insert(ucVO);  //  기존 회원 에게 추천인 쿠폰을 지급 하기.
		}
	...
	/*
	 * 요청 값: VO에 존재 : mPhone / 세션: X / VO에 없음: X 
	 * 요청 페이지: searchID.jsp 
	 * 리턴 값: model :mPhone(mVO.getmPhone) / 세션 : X 
	 * 기능: 아이디 찾기
	 */
	...
	public static String makeSignature(String url, String timestamp, String method, String accessKey, String secretKey)
		...
		String message = 
				new StringBuilder().append(method).append(space).append(url).append(newLine).append(timestamp).append(newLine).append(accessKey).toString();
	...
	/*
	 * 요청 값 : VO에 존재 : X / 세션 : mID, cart / VO에 없음 : X 
	 * 요청 페이지 : login.tag (공통) 
	 * 리턴 값 : model : X / 세션 : X 
	 * 기능 : 로그아웃
	 */
