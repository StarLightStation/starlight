	...
	<div class="content-wrapper">
		...
		<div class="container-xxl flex-grow-1 container-p-y">
			<h4 class="fw-bold py-3 mb-4">
				<span class="text-muted fw-light">상품 관리 /</span> 상품 편집
			</h4>
					...
					<div class="card mb-4">
						...
						<div class="card-body">
							<div class="d-flex align-items-start align-items-sm-center gap-4">
								...
								<img src="assets/img/products/${pdata.pImage}"
									alt="user-avatar" class="d-block rounded" height="240"
									width="210" id="uploadedAvatar" />
									 <img
									src="assets/img/products/${pdata.pImagedetail}"
									alt="user-avatar" class="d-block rounded" height="240"
						...
						<hr class="my-0" />
						<div class="card-body">
							<form action="adminProductUpdate.do?pNum=${pdata.pNum }"
								...
								<div class="row">
									...
									<div class="mb-3 col-md-6">
										<label class="form-label" for="country">카테고리</label> <select
											id="country" name="pCategory" class="select2 form-select">

											<option value="과실주"
												<c:if test="${pdata.pCategory eq '과실주'}">selected</c:if>>과실주</option>

											<option value="증류주"
												<c:if test="${pdata.pCategory eq '증류주'}">selected</c:if>>증류주</option>

											<option value="청주"
												<c:if test="${pdata.pCategory eq '청주'}">selected</c:if>>청주</option>

											<option value="탁주"
												<c:if test="${pdata.pCategory eq '탁주'}">selected</c:if>>탁주</option>


										</select>
									</div>
									<div class="mb-3 col-md-6">
										<label for="language" class="form-label">신맛</label> <select
											id="language" name="pSour" class="select2 form-select">
											<option value="약"
												<c:if test="${pdata.pSour eq '약'}">selected</c:if>>약</option>
											<option value="중"
												<c:if test="${pdata.pSour eq '중'}">selected</c:if>>중</option>
											<option value="강"
												<c:if test="${pdata.pSour eq '강'}">selected</c:if>>강</option>
										</select>
									</div>
									<div class="mb-3 col-md-6">
										<label for="timeZones" class="form-label">단맛</label> <select
											id="timeZones" name="pSweet" class="select2 form-select">
											<option value="약"
												<c:if test="${pdata.pSweet eq '약'}">selected</c:if>>약</option>
											<option value="중"
												<c:if test="${pdata.pSweet eq '중'}">selected</c:if>>중</option>
											<option value="강"
												<c:if test="${pdata.pSweet eq '강'}">selected</c:if>>강</option>
										</select>
									</div>
									<div class="mb-3 col-md-6">
										<label for="currency" class="form-label">탄산</label> <select
											id="currency" name="pSparkle" class="select2 form-select">
											<option value="약"
												<c:if test="${pdata.pSparkle eq '약'}">selected</c:if>>약</option>
											<option value="중"
												<c:if test="${pdata.pSparkle eq '중'}">selected</c:if>>중</option>
											<option value="강"
												<c:if test="${pdata.pSparkle eq '강'}">selected</c:if>>강</option>
										</select>
									</div>
								</div>
								<div class="mt-2">
